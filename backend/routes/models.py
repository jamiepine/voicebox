...
async def get_model_status():
    ...
    try:
        ...
        custom_entries = custom_models.list_custom_models()
        ...
        logger.warning("Failed to inject custom model status", exc_info=True)
...
async def list_custom_models_endpoint():
    ...
    return {"models": custom_models.list_custom_models()}


@router.post("/custom-models")
async def add_custom_model_endpoint(data: models.CustomModelCreate):
    """Register a new HuggingFace custom model (does NOT trigger download)."""
    try:
        result = custom_models.add_custom_model(
        ...
        raise HTTPException(status_code=500, detail=f"Failed to add custom model: {str(e)}") from e


@router.delete("/custom-models/{model_id}")
async def remove_custom_model_endpoint(model_id: str):
    """Unregister a custom model (does NOT delete cached files)."""
    try:
        removed = custom_models.remove_custom_model(model_id)
        if not removed:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to remove custom model: {str(e)}") from e
