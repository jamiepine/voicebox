...
from backend.tests.conftest import FakeLLM, _agent_payload, _contact
...
async def test_opt_out_ends_call_and_blocks_number_everywhere(db, agent, fake_llm, fake_tts):
    ...
    # The model was never asked for a reply — only for the post-call summary / analysis.
    assert all(c["system"] in (prompts.SUMMARY_SYSTEM, prompts.ANALYSIS_SYSTEM) for c in fake_llm.calls)
